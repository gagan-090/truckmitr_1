...
>;
type UploadDocumentsRouteProp = RouteProp<
  NavigatorParams,
  typeof STACKS.UPLOAD_DOCUMENTS
>;
...
export default function DocumentUploadScreen() {
  ...
  const submitDocuments = async () => {
    ...
    const combinedData = {
      ...
      aadharPhoto: userEdit?.aadharImagePath?.path
        ? userEdit?.aadharImagePath
        : {},
      drivingLicensePhoto: userEdit?.drivingLicensePath?.path
        ? userEdit?.drivingLicensePath
        : {},
    } as any;
    ...
    if (
      combinedData?.profile?.path &&
      combinedData?.profile?.mime &&
      combinedData?.profile?.filename
    ) {
    ...
    if (
      combinedData?.drivingLicensePhoto?.path &&
      combinedData?.drivingLicensePhoto?.mime &&
      combinedData?.drivingLicensePhoto?.filename
    ) {
    ...
    try {
      ...
      if (response?.data?.status) {
        ...
        if (profile?.data?.status) {
          ...
          // Navigate back to verification screen to refresh status
      ...
      >
        ...
        >
          ...
          <View>
            ...
            >
              {t(`aadharNumber`)}{' '}
              <Text style={{ color: colors.roseRed, fontWeight: 'bold' }}>
                *
              </Text>
            </Text>
            <TextInput
