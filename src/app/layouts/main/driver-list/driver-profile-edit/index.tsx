...
export default function DriverProfileEditByTransporter() {
    ...
    const dateOfBirth = driverProfileEdit?.DOB ? new Date(driverProfileEdit?.DOB) : moment().subtract(18, 'years').toDate();
    ...
    const validate = (): boolean => {
        ...
        if (!driverProfileEdit?.name) {
            newErrors.fullName = t('fullNameRequired');
            valid = false;
        }
        if (!driverProfileEdit?.email) {
            newErrors.email = t('e-mailRequired');
            valid = false;
        } 
        else if (driverProfileEdit?.email) {
            const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            if (!emailRegex.test(driverProfileEdit?.email)) {
                newErrors.email = t('invalidEmailFormat');
                valid = false;
            }
        }
        if (!driverProfileEdit?.mobile) {
            newErrors.mobile = t('mobileNumberRequired');
            valid = false;
        } else if (driverProfileEdit?.mobile.length < 10) {
            newErrors.mobile = t('mobileNumber_10_digits');
            valid = false;
        }
        if (!driverProfileEdit?.DOB) {
            newErrors.dateOfBirth = t('dobRequired');
            valid = false;
        }
        if (!driverProfileEdit?.Sex) {
            newErrors.gender = t('genderRequired');
            valid = false;
        }
        if (!driverProfileEdit?.address) {
            newErrors.address = t('addressRequired');
            valid = false;
        }
        if (!driverProfileEdit?.city) {
            newErrors.city = t('cityRequired');
            valid = false;
        }
        if (!driverProfileEdit?.states) {
    ...
    const _navigateDrivingDetails = () => {
        if (!validate()) return;
        navigation.navigate(STACKS.DRIVER_DRIVING_DETAILS_BY_TRANSPORTER, {
            profile: driverProfileEdit?.profilePath?.path ? driverProfileEdit?.profilePath : {},
            fullName: driverProfileEdit?.name,
            email: driverProfileEdit?.email,
            mobile: driverProfileEdit?.mobile,
            fatherName: driverProfileEdit?.Father_Name,
            dateOfBirth: moment(dateOfBirth).format("DD-MM-YYYY"),
            gender: driverProfileEdit?.Sex,
            maritalStatus: driverProfileEdit.Marital_Status,
            education: driverProfileEdit?.Highest_Education,
            address: driverProfileEdit?.address,
            city: driverProfileEdit?.city,
            state: driverProfileEdit?.states
        });
    };
    ...
    const _openCamera = async () => {
        ...
        ImagePicker.openCamera({
            ...
            .then(image => {
                ...
                dispatch(driverProfileEditAction({ ...driverProfileEdit, profilePath: image }));
    ...
    const _onPressProfileDelete = () => {
        dispatch(driverProfileEditAction({ ...driverProfileEdit, profilePath: {} }));
    }
    ...
    const profileImageUri = driverProfileEdit?.profilePath?.path ? driverProfileEdit?.profilePath?.path : driverProfileEdit?.images ? `${BASE_URL}public/${driverProfileEdit?.images}` : 'https://cdn-icons-png.flaticon.com/512/3177/3177440.png';
        ...
        <View style={{ flex: 1, backgroundColor: colors.white, alignItems: 'center' }}>
            ...
            <KeyboardAwareScrollView
                ...
                <TouchableOpacity onPress={_onPressProfile} activeOpacity={1}>
                    ...
                    />
                    {!driverProfileEdit?.profilePath?.path ? <TouchableOpacity
                ...
                <View style={{ flex: 1, width: responsiveWidth(100), paddingHorizontal: responsiveWidth(5) }}>
                    ...
                    <View>
                        ...
                        <TextInput
                            value={driverProfileEdit?.name || ''}
                            onChangeText={(text) => {
                                dispatch(driverProfileEditAction({ ...driverProfileEdit, name: text }));
                                setErrors((prevData) => ({
                                    ...prevData,
                                    fullName: undefined,
                                }));
                    ...
                    {/* E-mail */}
                    <View>
                        <Text style={{ color: colors.blackOpacity(0.9), fontSize: responsiveFontSize(1.7), fontWeight: '600' }}>{t('e-mail')} <Text style={{ color: colors.roseRed, fontWeight: 'bold' }}>*</Text>
                        </Text>
                        <TextInput
                            value={driverProfileEdit?.email || ''}
                            onChangeText={(text) => {
                                const lower = text.toLowerCase(); // lowercase all
                                dispatch(driverProfileEditAction({ ...driverProfileEdit, email: lower }));
                                setErrors((prevData) => ({
                                    ...prevData,
                                    email: undefined,
                                }));
                    ...
                    <View>
                        ...
                        <TextInput
                            value={driverProfileEdit?.mobile || ''}
                            editable={false}
                            placeholder={t('enterMobile')}
                            onChangeText={(text) => {
                                dispatch(driverProfileEditAction({ ...driverProfileEdit, mobile: text }));
                                setErrors((prevData) => ({
                                    ...prevData,
                                    mobile: undefined,
                                }));
                    ...
                    <TextInput
                        value={driverProfileEdit?.Father_Name || ''}
                        onChangeText={(text) => {
                            dispatch(driverProfileEditAction({ ...driverProfileEdit, Father_Name: text }));
                        }}
                    ...
                    <View>
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center', height: responsiveHeight(5.5), borderColor: colors.blackOpacity(0.2), borderWidth: 1, borderRadius: 10, marginTop: responsiveFontSize(0.5), paddingHorizontal: responsiveFontSize(2), }}>
                            <TextInput
                                value={driverProfileEdit?.DOB ? moment(dateOfBirth).format("DD-MM-YYYY") : ''}
                                editable={false}
                    ...
                    <View>
                        ...
                        <View style={{ flexDirection: 'row', marginTop: responsiveFontSize(0.5) }}>
                            <TouchableOpacity
                                onPress={() => {
                                    dispatch(driverProfileEditAction({ ...driverProfileEdit, Sex: 'Male' }));
                                    setErrors((prevData) => ({
                                        ...prevData,
                                        gender: undefined,
                                    }));
                                ...
                                style={{
                                    flexDirection: 'row',
                                    alignItems: 'center',
                                    backgroundColor: driverProfileEdit?.Sex === 'Male' ? colors.blueOpacity(0.2) : colors.blackOpacity(0.05),
                                ...
                                />
                                <Text style={{ color: colors.black, fontWeight: driverProfileEdit?.Sex === 'Male' ? '500' : '400', fontSize: responsiveFontSize(1.7), marginStart: responsiveFontSize(0.5) }}>
                                    Male
                                </Text>
                            </TouchableOpacity>
                            ...
                            <TouchableOpacity
                                onPress={() => {
                                    dispatch(driverProfileEditAction({ ...driverProfileEdit, Sex: 'Female' }));
                                    setErrors((prevData) => ({
                                        ...prevData,
                                        gender: undefined,
                                    }));
                                ...
                                style={{
                                    flexDirection: 'row',
                                    alignItems: 'center',
                                    backgroundColor: driverProfileEdit?.Sex === 'Female' ? colors.blueOpacity(0.2) : colors.blackOpacity(0.05),
                                    paddingHorizontal: responsiveFontSize(2.5),
                                    paddingVertical: responsiveFontSize(1),
                                    borderRadius: 100,
                                }}>
                                ...
                                />
                                <Text style={{ color: colors.black, fontWeight: driverProfileEdit?.Sex === 'Female' ? '500' : '400', fontSize: responsiveFontSize(1.7), marginStart: responsiveFontSize(0.5) }}>
                                    Female
                                </Text>
                            </TouchableOpacity>
                        </View>
                    ...
                    <View style={{ flexDirection: 'row', marginTop: responsiveFontSize(0.5) }}>
                        <TouchableOpacity
                            onPress={() => {
                                dispatch(driverProfileEditAction({ ...driverProfileEdit, Marital_Status: 'Single' }));
                            }}
                            style={{
                                flexDirection: 'row',
                                alignItems: 'center',
                                backgroundColor: driverProfileEdit.Marital_Status === 'Single' ? colors.blueOpacity(0.2) : colors.blackOpacity(0.05),
                                paddingHorizontal: responsiveFontSize(2),
                                paddingVertical: responsiveFontSize(1),
                                borderRadius: 100,
                            }}>
                            <Text style={{ color: colors.black, fontWeight: driverProfileEdit.Marital_Status === 'Single' ? '500' : '400', fontSize: responsiveFontSize(1.7) }}>Single</Text>
                        </TouchableOpacity>
                        <Space width={responsiveFontSize(1.5)} />
                        <TouchableOpacity
                            onPress={() => {
                                dispatch(driverProfileEditAction({ ...driverProfileEdit, Marital_Status: 'Married' }));
                            }}
                            style={{
                                flexDirection: 'row',
                                alignItems: 'center',
                                backgroundColor: driverProfileEdit.Marital_Status === 'Married' ? colors.blueOpacity(0.2) : colors.blackOpacity(0.05),
                                paddingHorizontal: responsiveFontSize(2),
                                paddingVertical: responsiveFontSize(1),
                                borderRadius: 100,
                            }}>
                            <Text style={{ color: colors.black, fontWeight: driverProfileEdit.Marital_Status === 'Married' ? '500' : '400', fontSize: responsiveFontSize(1.7) }}>Married</Text>
                        </TouchableOpacity>
                        <Space width={responsiveFontSize(1.5)} />
                        <TouchableOpacity
                            onPress={() => {
                                dispatch(driverProfileEditAction({ ...driverProfileEdit, Marital_Status: 'Widowed' }));
                            }}
                            style={{
                                flexDirection: 'row',
                                alignItems: 'center',
                                backgroundColor: driverProfileEdit.Marital_Status === 'Widowed' ? colors.blueOpacity(0.2) : colors.blackOpacity(0.05),
                                paddingHorizontal: responsiveFontSize(2),
                                paddingVertical: responsiveFontSize(1),
                                borderRadius: 100,
                            }}>
                            <Text style={{ color: colors.black, fontWeight: driverProfileEdit.Marital_Status === 'Widowed' ? '500' : '400', fontSize: responsiveFontSize(1.7) }}>Widowed</Text>
                        </TouchableOpacity>
                    </View>
                    <View style={{ flexDirection: 'row', marginTop: responsiveFontSize(1) }}>
                        <TouchableOpacity
                            onPress={() => {
                                dispatch(driverProfileEditAction({ ...driverProfileEdit, Marital_Status: 'Divorced' }));
                            }}
                            style={{
                                flexDirection: 'row',
                                alignItems: 'center',
                                backgroundColor: driverProfileEdit.Marital_Status === 'Divorced' ? colors.blueOpacity(0.2) : colors.blackOpacity(0.05),
                                paddingHorizontal: responsiveFontSize(2),
                                paddingVertical: responsiveFontSize(1),
                                borderRadius: 100,
                            }}>
                            <Text style={{ color: colors.black, fontWeight: driverProfileEdit.Marital_Status === 'Divorced' ? '500' : '400', fontSize: responsiveFontSize(1.7) }}>Divorced</Text>
                        </TouchableOpacity>
                    </View>
                    <Space height={responsiveFontSize(2.5)} />
                    ...
                    <View style={{ flexDirection: 'row', marginTop: responsiveFontSize(0.5) }}>
                        <TouchableOpacity
                            onPress={() => {
                                dispatch(driverProfileEditAction({ ...driverProfileEdit, Highest_Education: 'Below 10th' }));
                            }}
                            style={{
                                flexDirection: 'row',
                                alignItems: 'center',
                                backgroundColor: driverProfileEdit?.Highest_Education === 'Below 10th' ? colors.blueOpacity(0.2) : colors.blackOpacity(0.05),
                                paddingHorizontal: responsiveFontSize(2),
                                paddingVertical: responsiveFontSize(1),
                                borderRadius: 100,
                            }}>
                            <Text style={{ color: colors.black, fontWeight: driverProfileEdit?.Highest_Education === 'Below 10th' ? '500' : '400', fontSize: responsiveFontSize(1.7), marginStart: responsiveFontSize(0.5) }}>Below 10th</Text>
                        </TouchableOpacity>
                        <Space width={responsiveFontSize(1.5)} />
                        <TouchableOpacity
                            onPress={() => {
                                dispatch(driverProfileEditAction({ ...driverProfileEdit, Highest_Education: '10th Passed' }));
                            }}
                            style={{
                                flexDirection: 'row',
                                alignItems: 'center',
                                backgroundColor: driverProfileEdit?.Highest_Education === '10th Passed' ? colors.blueOpacity(0.2) : colors.blackOpacity(0.05),
                                paddingHorizontal: responsiveFontSize(2),
                                paddingVertical: responsiveFontSize(1),
                                borderRadius: 100,
                            }}>
                            <Text style={{ color: colors.black, fontWeight: driverProfileEdit?.Highest_Education === '10th Passed' ? '500' : '400', fontSize: responsiveFontSize(1.7), marginStart: responsiveFontSize(0.5) }}>10th Passed</Text>
                        </TouchableOpacity>
                        <Space width={responsiveFontSize(1.5)} />
                        <TouchableOpacity
                            onPress={() => {
                                dispatch(driverProfileEditAction({ ...driverProfileEdit, Highest_Education: '12th Passed' }));
                            }}
                            style={{
                                flexDirection: 'row',
                                alignItems: 'center',
                                backgroundColor: driverProfileEdit?.Highest_Education === '12th Passed' ? colors.blueOpacity(0.2) : colors.blackOpacity(0.05),
                                paddingHorizontal: responsiveFontSize(2),
                                paddingVertical: responsiveFontSize(1),
                                borderRadius: 100,
                            }}>
                            <Text style={{ color: colors.black, fontWeight: driverProfileEdit?.Highest_Education === '12th Passed' ? '500' : '400', fontSize: responsiveFontSize(1.7), marginStart: responsiveFontSize(0.5) }}>12th Passed</Text>
                        </TouchableOpacity>
                    </View>
                    <View style={{ flexDirection: 'row', marginTop: responsiveFontSize(1) }}>
                        <TouchableOpacity
                            onPress={() => {
                                dispatch(driverProfileEditAction({ ...driverProfileEdit, Highest_Education: 'Graduate' }));
                            }}
                            style={{
                                flexDirection: 'row',
                                alignItems: 'center',
                                backgroundColor: driverProfileEdit?.Highest_Education === 'Graduate' ? colors.blueOpacity(0.2) : colors.blackOpacity(0.05),
                                paddingHorizontal: responsiveFontSize(2),
                                paddingVertical: responsiveFontSize(1),
                                borderRadius: 100,
                            }}>
                            <Text style={{ color: colors.black, fontWeight: driverProfileEdit?.Highest_Education === 'Graduate' ? '500' : '400', fontSize: responsiveFontSize(1.7), marginStart: responsiveFontSize(0.5) }}>Graduate</Text>
                        </TouchableOpacity>
                        <Space width={responsiveFontSize(1.5)} />
                        <TouchableOpacity
                            onPress={() => {
                                dispatch(driverProfileEditAction({ ...driverProfileEdit, Highest_Education: 'Post Graduate' }));
                            }}
                            style={{
                                flexDirection: 'row',
                                alignItems: 'center',
                                backgroundColor: driverProfileEdit?.Highest_Education === 'Post Graduate' ? colors.blueOpacity(0.2) : colors.blackOpacity(0.05),
                                paddingHorizontal: responsiveFontSize(2),
                                paddingVertical: responsiveFontSize(1),
                                borderRadius: 100,
                            }}>
                            <Text style={{ color: colors.black, fontWeight: driverProfileEdit?.Highest_Education === 'Post Graduate' ? '500' : '400', fontSize: responsiveFontSize(1.7), marginStart: responsiveFontSize(0.5) }}>Post Graduate</Text>
                        </TouchableOpacity>
                    </View>
                    <Space height={responsiveFontSize(2.5)} />
                    ...
                    <View>
                        ...
                        <TextInput
                            value={driverProfileEdit?.address || ''}
                            multiline
                            onChangeText={(text) => {
                                dispatch(driverProfileEditAction({ ...driverProfileEdit, address: text }));
                                setErrors((prevData) => ({
                                    ...prevData,
                                    address: undefined,
                                }));
                    ...
                    <View>
                        ...
                        <TextInput
                            value={driverProfileEdit?.city || ''}
                            onChangeText={(text) => {
                                dispatch(driverProfileEditAction({ ...driverProfileEdit, city: text }));
                                setErrors((prevData) => ({
                                    ...prevData,
                                    city: undefined,
                                }));
                    ...
                    <View>
                        ...
                        <Dropdown
                            ...
                            value={(locations?.find(state => driverProfileEdit?.states && (state.name.toLowerCase() === driverProfileEdit.states.toLowerCase() || state.id === Number(driverProfileEdit.states) || state?.codes === driverProfileEdit?.states))?.id)?.toString() || ''}
                            onChange={item => {
                                dispatch(driverProfileEditAction({ ...driverProfileEdit, states: item.value }));
                                setErrors((prevData) => ({
                                    ...prevData,
                                    state: undefined,
                                }));
                        ...
                        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.blackOpacity(.7), }}>
                            <View style={{ backgroundColor: colors.white, alignItems: 'center', width: responsiveWidth(90), borderRadius: 10, overflow: 'hidden' }}>
                                ...
                                <DatePicker
                                    ...
                                    onDateChange={(date) => {
                                        dispatch(driverProfileEditAction({ ...driverProfileEdit, DOB: date }));
                                        setErrors((prevData) => ({
                                            ...prevData,
