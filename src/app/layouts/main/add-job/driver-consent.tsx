...
export default function DriverConsent() {
    ...
    useEffect(() => {
        const _fetchPolicy = async () => {
            try {
                setLoading(true);
                const response: any = await axiosInstance.get(END_POINTS?.DRIVER_CONSENT);
    ...
    return (
        <View style={{ flex: 1, backgroundColor: colors.white }}>
            ...
            <View style={{ flexDirection: 'row', width: '100%', alignItems: 'center', padding: responsiveWidth(3) }}>
                ...
                <Text style={{ width: responsiveWidth(100), fontSize: responsiveFontSize(2.2), color: colors.royalBlue, fontWeight: 'bold', textAlign: 'center', position: 'absolute', zIndex: 1 }}>{`Driver Consent`}</Text>
            </View>
